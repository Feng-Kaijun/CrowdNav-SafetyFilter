...
class SafetyFilter(object):
    ...
    def compuate_safe_set(self, self_state:FullState, human_states):
        t1 = time.time()
        # irisRegion = self.cirle_state_constrain(self_state, human_states)
        irisRegion = self.state_constrain(self_state, human_states)
        # irisRegion = self.iris_state_constrain(self_state, human_states)
        ...
        if self.system == "holonomic":
            ...
            # local_goal_state = self.get_loacl_goal(self_state, human_states, safe_set, points)
            ...
            points = np.array(safe_set.getVertices())
            self.output['safe_set'] = points[:, 0:2]
            # safe_set_proj = safe_set.projection(1)
            # local_goal_state = self.get_loacl_goal(self_state, human_states, safe_set_proj)
            # local_goal_state = self.get_minpotential_goal(self_state, human_states, safe_set_proj)

        # self.output['local_goal'] = local_goal_state
    ...
    def compuate_safe_action(self, self_state:FullState, current_action, human_states):
        ...
        if self.system == "holonomic":
            ...
            # goal_state = self.output['local_goal'].reshape(2, 1)
            current_input = np.array([[current_action.vx], [current_action.vy]])
        elif self.system == "unicycle":
            current_state = np.array([[self_state.px], [self_state.py], [self_state.theta]])
            goal_state = np.array([[self_state.gx], [self_state.gy], [0]])
            # goal_state = self.output['local_goal'].reshape(3, 1)
            # if goal_state[-1] == -1:
            #     return ActionRot(0, 0)
            current_input = np.array([[current_action.v], [current_action.r / self.T]])
        # n = input_poly.getDimNums()
        # m = input_poly.getConNums() + safe_set.getConNums()
        # P = 2E + 2BB
        rho = 1
        # P = 2 * np.eye(n) + 2 * rho * np.dot(systemB.T, systemB)
        # Q = 2 * rho * np.dot(current_state.T, systemA.T).dot(systemB) - 2 * current_input.T - 2 * np.dot(goal_state.T, systemB) * rho
        ...
        # for action in self.np_action_space:
        for action in self.free_action_space:
            constrain_result = A.dot(action) - Aub
            if constrain_result.max() <= 0:
                # object_value = 0.5 * np.dot(action.T, P).dot(action) + Q.dot(action)
                ...
                # second = next_state - goal_state
                # second = next_state[2] - goal_state[2]
                object_value = np.linalg.norm(first) + rho * np.linalg.norm(second)

                # second = self.get_collision_cost(self_state, next_state, action, human_states, 2)
                # object_value = np.linalg.norm(first) + rho * second
    ...
    def compute_stage_cost(self, R_state, P_states, dist=0.0):
        Rx = R_state[0]
        Ry = R_state[1]
        stage_cost = 0
    ...
    def iris_state_constrain(self, self_state, human_states):
        bounds = irispy.Polyhedron.from_bounds([self.lb, self.lb], [self.ub, self.ub])
        start = np.array([self_state.px, self_state.py])
        obstacles = []

        for human_state in human_states:
            center = np.array([human_state.px, human_state.py]).reshape(2, 1)
            radius = human_state.radius + self_state.radius
            # Vtheta is between 0 -> 2pi
            if human_state.vx == 0 and human_state.vy == 0:
                Vtheta = None
            else:
                Vtheta = math.atan2(human_state.vy, human_state.vx)

            if Vtheta == None:
                vertex_P1 = np.array([center[0, 0] + radius, center[1, 0] + radius]).reshape(2, 1)
                vertex_P2 = np.array([center[0, 0] + radius, center[1, 0] - radius]).reshape(2, 1)
                vertex_P3 = np.array([center[0, 0] - radius, center[1, 0] + radius]).reshape(2, 1)
                vertex_P4 = np.array([center[0, 0] - radius, center[1, 0] - radius]).reshape(2, 1)
                points = np.hstack((vertex_P1, vertex_P2, vertex_P3, vertex_P4))
            else:
                Vlength = math.sqrt(math.pow(human_state.vx, 2) + math.pow(human_state.vy, 2))
                length = self.K * Vlength
                tan_phi = math.tan(self.phi)
                intercept = float(radius * math.sqrt(math.pow(tan_phi, 2) + 1))
                # [x; y]
                vertex_P1 = np.array([radius + length, (radius + length) * tan_phi + intercept]).reshape(2, 1)
                vertex_P2 = np.array([vertex_P1[0, 0], -vertex_P1[1, 0]]).reshape(2, 1)
                vertex_P3 = np.array([-radius, -radius * tan_phi + intercept]).reshape(2, 1)
                vertex_P4 = np.array([vertex_P3[0, 0], -vertex_P3[1, 0]]).reshape(2, 1)

                rot_matrix = np.array([[math.cos(Vtheta), - math.sin(Vtheta)], [math.sin(Vtheta), math.cos(Vtheta)]])
                vertex_P1 = np.dot(rot_matrix, vertex_P1) + center
                vertex_P2 = np.dot(rot_matrix, vertex_P2) + center
                vertex_P3 = np.dot(rot_matrix, vertex_P3) + center
                vertex_P4 = np.dot(rot_matrix, vertex_P4) + center
                points = np.hstack((vertex_P1, vertex_P2, vertex_P3, vertex_P4))

            obstacles.append(points)
        region = irispy.inflate_region(obstacles, start, bounds=bounds, require_containment=True)

        A = region.getPolyhedron().getA()
        B = region.getPolyhedron().getB()
        vertexs = irispy.Polyhedron.getDrawingVertices(region.getPolyhedron())

        obstacle_hullPoints = np.zeros([5, 4, 2])
        for i in range(5):
            tmp = obstacles[i].T
            obstacle_hullPoints[i, :, :] = tmp

        self.output['iris_vertexs'] = vertexs
        self.output['obstacle_hullPoints'] = obstacle_hullPoints
        if self.system == "holonomic":
            irisRegion = Polyhedron(A, B)
            A_t = irisRegion.getA()
            b_t = irisRegion.getB()
        elif self.system == "unicycle":
            irisRegion = Polyhedron(A, B)
            ...
            irisRegion = irisRegion.addDimensions(theta_cons)
            A_t = irisRegion.getA()
            b_t = irisRegion.getB()
        self.output['iris_poly'] = irisRegion
        return irisRegion

    def closest_collion_constrain(self, self_state, human_states):
        # compute the closest distance between robot and humans
        dmin = float('inf')
        closest_human = None
        for i, human in enumerate(human_states):
            px = human.px - self_state.px
            py = human.py - self_state.py
            closest_dist = math.sqrt(math.pow(px, 2) + math.pow(py, 2)) - human.radius - self_state.radius
            if closest_dist < dmin:
                dmin = closest_dist
                closest_human = human
        a = (closest_human.py - self_state.py) / (closest_human.px - self_state.px)
        b = self_state.py - closest_human.py
        R = self_state.radius + closest_human.radius + 0.2
        A = 1 + math.pow(a, 2)
        B = -2 * closest_human.px - 2 * math.pow(a, 2) * self_state.px + 2 * a * b
        C = math.pow(closest_human.px, 2) + math.pow(a, 2) * math.pow(self_state.px, 2) - 2 * a * b * self_state.px + math.pow(b, 2) - math.pow(R, 2)
        x1 = (-B + math.sqrt(math.pow(B, 2) - 4 * A * C)) / (2 * A)
        y1 = a * (x1 - self_state.px) + self_state.py
        x2 = (-B - math.sqrt(math.pow(B, 2) - 4 * A * C)) / (2 * A)
        y2 = a * (x2 - self_state.px) + self_state.py
        if math.pow(self_state.px - x1, 2) + math.pow(self_state.py - y1, 2) < math.pow(self_state.px - x2, 2) + math.pow(self_state.py - y2, 2):
            sx = x1
            sy = y1
        else:
            sx = x2
        ...
        k = - 1 / a
        d = sy - k * sx
        if self_state.py - k * self_state.px - d <= 0:
            collion_constrain_A = np.array([[-k, 1]])
            collion_constrain_b = np.array([[d]])
        else:
            collion_constrain_A = np.array([[k, -1]])
            collion_constrain_b = np.array([[-d]])
        return Polyhedron(collion_constrain_A, collion_constrain_b)
    ...
    def BVC_collion_constrain(self, self_state:FullState, human_states):
        # compute the BVC of robot
        Pi = np.array([[self_state.px], [self_state.py]])
        Ri = self_state.radius * 1 - self_state.radius * 1
        A = np.zeros((len(human_states), 2))
        b = np.zeros((len(human_states), 1))
        for i, human_state in enumerate(human_states):
            Pj = np.array([[human_state.px], [human_state.py]])
            Pij = Pi - Pj
            A[i, :] = Pij.T
            b[i, :] = np.dot((Pi.T + Pj.T) / 2, Pij) - Ri * np.linalg.norm(Pij)

        result = Polyhedron(-A, -b)
        return  result
    ...
    def state_constrain(self, self_state, human_states):
        ...
        # next_human_states = human_states
        # for next_human_state in next_human_states:
        #     next_human_state.px = next_human_state.px + next_human_state.vx * self.T
        #     next_human_state.py = next_human_state.py + next_human_state.vy * self.T

        # collion_cons_now = self.closest_collion_constrain(self_state, human_states)
        # state_constrain = state_constrain.addConstraints(collion_cons_now.getA(), collion_cons_now.getB())
        ...
        # BVC_collion_constrain = self.BVC_collion_constrain(self_state, human_states)
        # state_constrain = state_constrain.addConstraints(BVC_collion_constrain.getA(), BVC_collion_constrain.getB())
